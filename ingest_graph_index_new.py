        ...
        try:
            ...
            for coll_name in ['Class', 'Method', 'Function', 'File']:
                if self.db.has_collection(coll_name):
                    ...
                    # Count documents with enhanced fields
                    full_context_count = collection.count(
                        filters={'full_context': {'$exists': True}}
                    ) if hasattr(collection, 'count') else 0
                    
                    summary_count = collection.count(
                        filters={'summary': {'$exists': True}}
                    ) if hasattr(collection, 'count') else 0
