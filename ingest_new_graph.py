        ...
        try:
            ...
            for coll_name in ['Class', 'Method', 'Function', 'File']:
                if self.db.has_collection(coll_name):
                    ...
                    try:
                        # Query to count documents with specific fields
                        aql_full_context = f"""
                        FOR doc IN {coll_name}
                            ...
                            RETURN length
                        """
                        full_context_count = list(self.db.aql.execute(aql_full_context))[0] if self.db.aql.execute(aql_full_context) else 0
                        ...
                        FOR doc IN {coll_name}
                            ...
                            RETURN length
                        """
                        summary_count = list(self.db.aql.execute(aql_summary))[0] if self.db.aql.execute(aql_summary) else 0
